...
namespace Tara {
	...
	namespace Utility {
		...
		std::vector<char> Zip::GetFileData_Char(std::string filename)
		{
			...
			i.assign((char*)data, (char*)data + size);
			return i;
		}
		std::vector<uint8_t> Zip::GetFileData_UChar(std::string filename)
		{
			...
			i.assign((uint8_t*)data, (uint8_t*)data + size);
			return i;
		}
		...
		void* Zip::GetFileData_Void(std::string filename)
		{
			...
			TARA_DEBUG("Reading: %s", filename.c_str());
			if (c->size == 0) {
				TARA_DEBUG("Loading...: %s", filename.c_str());
				LoadZipContent(filename, c);
			}
			return c->data;
		}
		...
		bool Zip::CheckDataIsLoaded(std::string filename) {
			if (!FileExist(filename)) return false;
			const void* data = GetFileData_Void(filename);
			return data != NULL;
		}
		void Zip::LoadData(std::string filename) {
		...
		bool Zip::Contain(ZipContent* content)
		{
			for (auto i : contents)
				if (i.second == content) return true;
			return false;
		}
		bool Zip::Contain(const char* uuid)
		{
			for (auto i : contents)
				if (strcmp(i.second->uuid, uuid)) return true;
			return false;
		}
		...
		Zip::Zip(const char* filepath) : IPackage()
		{
		...
		void Zip::OpenZipFile()
		{
			...
			while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
				...
				if (size > 0) {
#ifdef _TDEBUG
					TARA_DEBUG("filename: %s", filename.c_str());
#endif
					archive_read_data_skip(a);
					ZipContent* zc = new ZipContent(NULL, 0);
					contents[filename] = zc;
				}
			}
		...
		void Zip::LoadZipContent(std::string filename, ZipContent* content) {
			struct archive* a;
			struct archive_entry* entry;
			int r;
			...
			r = archive_read_open_filename(a, filepath, 10240); // Note 1
			while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
				std::string filename_c = archive_entry_pathname(entry);
				size_t size = archive_entry_size(entry);
				if (filename != filename_c || size == 0) {
					archive_read_data_skip(a);
					continue;
				}
#ifdef _TDEBUG
				TARA_DEBUG("\tloading file data: %s", filename_c.c_str());
#endif
				const void* data;
				la_int64_t offset = 0;
				...
				while (archive_read_data_block(a, &data, &size, &offset) != ARCHIVE_EOF) {
					size_t pre = total;
					total += size;
					// Get the current reading memory block
					char* vd = (char*)malloc(size);
					memcpy_s(vd, size, data, size);
					// Appending to the total memory block
					d = (char*)realloc(d, total);
					memcpy(&d[pre], vd, size);
#ifdef _TDEBUG
					TARA_DEBUG("\tsize: %zu, offset: %zu", size, offset);
#endif
				}
				content->data = d;
				content->size = total;
#ifdef _TDEBUG
				TARA_DEBUG("loading file finish: %zu", total);
#endif
				break;
			}
			r = archive_read_free(a);  // Note 3
