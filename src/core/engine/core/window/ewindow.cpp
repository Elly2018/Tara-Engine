	...
	void EWindow::Render() {
		...
		for (auto& light : lightSystem) {
			...
			case LightType::Direction:
				useMat = CommomMaterial().at(CommomMaterialType::DirectionalShadow);
				break;
			case LightType::Point:
				useMat = CommomMaterial().at(CommomMaterialType::PointShadow);
				break;
			}
		...
		for (auto& maincamera : useCam) {
			...
			if (renderer.wireframeMode) {
				...
				batchRenderer.DrawCall(CommomMaterial().at(CommomMaterialType::Color), true);
			}
