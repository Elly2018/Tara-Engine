...
namespace Tara {
	namespace UI {
        ...
        void PropertiesWindow::Content() {
            ...
            bool debug = psg.state_bool.GetState("debug", false);
            if (UI::ImGui_Checkbox("Debug", &debug)) {
                TARA_DEBUG("Toggle Debug");
            }
            psg.state_bool.SetState("debug", debug);

            ObjectBase* focusObject = psg.state_objectBase.GetState("FocusObject", NULL);
            if (!focusObject) {
                UI::ImGui_EndChild();
                return;
            }
            psg.state_objectBase.SetState("FocusObject", focusObject);

            // Scene object view
            if (static_cast<SceneObject*>(focusObject)) {
                SceneObject* so = static_cast<SceneObject*>(focusObject);
            ...
            // File object view
            else if (static_cast<FileBase*>(focusObject)) {
                FileBase* fb = static_cast<FileBase*>(focusObject);
                fb->DrawProp();
            }
            UI::ImGui_EndChild();
        }
