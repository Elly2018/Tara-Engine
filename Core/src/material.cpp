...
namespace Tara {
	AssetPool<Material> Material::m_materialPool = AssetPool<Material>();
	...
	Material::~Material()
	{
		m_materialPool.Remove(this);
	}

	void Material::Use()
	{
		if (m_shader == nullptr) {
			TARA_ERROR_LEVEL("Material has no shader binding: %s.", 4, m_name);
			return;
		}
		m_shader->Use();
	...
	void Material::SetTextureIndex(const char* name, int32_t v)
	{
	...
	void Material::SetNumber(const char* name, float_t v)
	{
		m_numberBind[name] = v;
	}
	void Material::SetVec2(const char* name, glm::vec2 v)
	{
		m_vec2Bind[name] = v;
	}
	void Material::SetVec3(const char* name, glm::vec3 v)
	{
		m_vec3Bind[name] = v;
	}
	...
	void Material::SetMat3(const char* name, glm::mat3 v)
	{
		m_mat3Bind[name] = v;
	}
	...
	void Material::UniformTexture(const char* location, uint32_t v)
	{
		if (v == -1) return;
		int32_t index = m_TextureIndex.at(location);
		glActiveTexture(GL_TEXTURE0 + index);
	...
	void Material::UniformNumber(const char* location, float_t v)
	{
		GLint i = m_shader->GetUniformLocation(location);
		glUniform1f(i, v);
	}
	void Material::UniformVec2(const char* location, glm::vec2 v)
	{
		GLint i = m_shader->GetUniformLocation(location);
		glUniform2f(i, v.x, v.y);
	}
	void Material::UniformVec3(const char* location, glm::vec3 v)
	{
		GLint i = m_shader->GetUniformLocation(location);
		glUniform3f(i, v.x, v.y, v.z);
	}
	void Material::UniformVec4(const char* location, glm::vec4 v)
	{
		GLint i = m_shader->GetUniformLocation(location);
		glUniform4f(i, v.x, v.y, v.z, v.w);
	}
	void Material::UniformMat3(const char* location, glm::mat3 v)
	{
		GLint i = m_shader->GetUniformLocation(location);
		glUniformMatrix3fv(i, 1, false, glm::value_ptr(v));
	}
	void Material::UniformMat4(const char* location, glm::mat4 v)
	{
		GLint i = m_shader->GetUniformLocation(location);
