...
namespace Tara {
	namespace UI {
		...
		void ImGui_WindowBase::Render()
		{
			if (show) {
				Start();
				ImGui_Begin(title.c_str(), &show, flag);
				Content();
				ImGui_End();
			}
		}
		...
		{
			if (show) {
				ImGui_OpenPopup(title.c_str(), pflags);
				Start();
				if (ImGui_BeginPopupModal(title.c_str(), &show, flag)) {
					Content();
					ImGui_EndPopup();
				}
			}
		...
		void ImGui_SceneView::RenderCamera(CameraComponent* target)
		{
		...
		void ImGui_SceneView::SetCamera(CameraComponent* target)
		{
			m_camera = target;
		}
		CameraComponent* ImGui_SceneView::GetCamera()
		{
			return m_camera;
		}
